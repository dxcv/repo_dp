...
def compute_yld_eplus(date, date_inic, serie_eplus, tacs, ):
    ...
    print(date_inic)
    print(date)
    days_betwwen = fs.get_dates_between(date_inic, date)
    print(serie_eplus)
    ...
    for afp in serie_eplus.columns:
        ...
        for day in days_betwwen:
            ...
            rent_list.append(100+(100*aux_rent))
            print(rent_list)
...
def compute_rent_afp(serie, date, tacs, afp=None):
    ...
    '''
    date_post = date+dt.timedelta(days=+1)
    rent = (serie[date_post]/serie[date])-1
    if afp=='FDIMEPS CI Equity':
    	rent = rent + compute_tacs(tacs, date_post,  'IMT E-PLUS')
...
if __name__ == "__main__":
    ...
    rentabilidad(date=date, 
                 ...
                 icp=icp,
                 date_eplus=dt.datetime(2017, 12, 29),)#dt.datetime(2017, 12, 28)
