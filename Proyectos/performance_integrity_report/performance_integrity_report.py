...
def rentabilidad(date):
    ...
    '''
    df_risk_hoy = compute_df_risk(date)
    #obtenemos la info de risk del dia anterios para sacar el valor cuota
    df_risk_ayer = compute_df_risk(get_previous_weekdays(date))
    df_risk = compute_df_final(df_risk_hoy, df_risk_ayer[['valor', 'tasa', 'duration']], risk=True)
    ...
    df_nacional = df.loc[df["tasa_hoy"].notnull(), ['moneda', 'codigo_fdo','weight','tasa_hoy','tasa_ayer', 'duration_ayer']]
...
def compute_rezagados(df_sql, df_risk):
    ...
    rezagados_df = rezagados_df.loc[rezagados_df["tasa_hoy"].isnull() & rezagados_df["precio_risk"].isnull()]
...
def compute_r(date, df, ins, nat):
    ...
    if nat is True:
        p1 = 'valor_ayer'
        p2 = 'valor_hoy'
        tasa_spot_risk = float(df.loc[ins]['tasa_hoy'])/100
    ...
    if instrument_return <= -0.01 and nat is True:
        tasa_yesterday_risk = float(df.loc[ins]['tasa_ayer'])/100
        durmod= float(df.loc[ins]['duration_ayer'])/(1+tasa_yesterday_risk)
...
def generar_excel(df_retorno_fondos, df_retorno_instrumentos, df_nacional, df_internacional, rezagados,name):
    ...
    df_internacional = df_internacional.rename(columns={'precio_risk':'precio_hoy', 'precio_sql':'precio_ayer'})
    print(df_internacional)
