...
def plot_series(series, title="Default Title", animated=False, xlabel="$t$", ylabel="return"):
    '''
    Grafica varias series de tiempo, puede ser animado para los precizos.
    '''
    ...
    if animated == True:
        ...
        ax.legend(names, loc="upper left", fancybox=True)
        for i in range (len(series[0])):
            for j in range(len(series)):
                dats[j].append(series[j].values[i])
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        tenors = series[0].index
        
        for serie in series:
            plt.plot(tenors, serie, linestyle='-', color=np.random.rand(3,))
        plt.legend(names)
        plt.show()