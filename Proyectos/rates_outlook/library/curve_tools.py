...
def compound_spot_curve(forward_curve, method, tpm_base, bonds_base):
    ...
    elif method == "compounded_mxn":
        spot_curve["yield"] = (((spot_curve["yield"])**(bonds_base/spot_curve["days"])) - 1.0) * 360/182
    elif method == "linear":
...
def dcf_mexico(rates, cash_flows, tenors, outstanding):
    
   
    pv = (cash_flows / (1+rates*182/360)**tenors).sum(axis=0)
    price = (pv/outstanding) * 100
    return price
...
def dcf_error_mexico(rate, cash_flows, tenors, outstanding, target_price):
    '''
    Es la funcion que calcula el error entre una valorizacion
    de una iteracion y el precio real del activo.
    '''
    price_iteration = dcf_mexico(rate, cash_flows, tenors, outstanding)
    error = price_iteration - target_price
    return error
...
def compute_zero_swap_curve(swap_instruments, long_end_date, method, bonds_base=360, tpm_base=360):
    ...
    # Pasamos a base 365 para valorizar 
...
def compute_valuation(curve_instruments, cash_flow_tables, zero_curves_cash, zero_curve_swap=None, mexico=False):
    '''
    ...
    #exit()
    dias_base_tasa = 365
    if mexico==True: #mexico usa tasa base anual compuesta semestral de 182 dias
        dias_base_tasa = 182
    zero_curves_cash["tenors"] = np.arange(len(zero_curves_cash.index))/dias_base_tasa 
    ...
    for ticker, instrument in curve_instruments.iterrows():
        ...
        #fs.print_full(zero_curves_cash)
        ...
        for scenario in zero_curves_scenarios:
            ...
            if mexico == True:
                ...
                #fs.print_full(total_cash_flow)
                #fs.print_full(tenors)
                # print(outstanding)
                valuation_price = dcf_mexico(curve, total_cash_flow, tenors, outstanding)
                x0 = 0.0775
                valuation_yield = optimize.newton(dcf_error_mexico, x0, args=(total_cash_flow, tenors, outstanding, valuation_price))
...
def compute_fras(curve_instruments, fra_limit, mexico=False):
    '''
    ...
    if mexico == True:
        factor = 182/360
        curve_instruments["fra"] = factor**(-1) * (((yld*factor+1)**(days_to_maturity/(delta_days)))/((shift_yld*factor+1)**(shift_days_to_maturity/(delta_days))) - 1.0)
    else:
