...
def get_liquidacion_transacciones(date):
    ...
    funds = get_FI_funds()
    query_iif = "Select Fecha, nemotecnico as nemo, vende, compra, captacion as monto from TransaccionesIIF where fecha_liq = 'AUTODATE' and ( compra in ("+ funds + ") or vende in ("+ funds + "))"
    query_iif  = query_iif.replace("AUTODATE",str(date))
    query_irf = "Select Fecha, instrumento as nemo, vende, compra, monto from TransaccionesIRF where fecha_liq = 'AUTODATE' and ( compra in ("+ funds + ") or vende in ("+ funds + "))"
