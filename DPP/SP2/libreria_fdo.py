...
def export_sheet_pdf_multiple_index(sheet_index, path_in, path_out):
	'''
	Exporta multiples hojas de Excel en PDF. 
	'''
	xlApp = client.Dispatch("Excel.Application")
	...
	for index in sheet_index:
		...
		print(index)
		print('************')
		#ws = books.get_sheet_by_name(index)
		ws = books.Worksheets[index]
		ws.PageSetup.Zoom = False
		ws.PageSetup.FitToPagesTall = 1
		ws.PageSetup.FitToPagesWide = 1
		#ws.PageSetup.PrintArea = print_area
	books.WorkSheets(sheet_index).Select()
	books.ActiveSheet.ExportAsFixedFormat(0, path_out)
	books.Close(True)
...
def plot_curve(curve, days):
    ...
    plt.plot(days, curve, linestyle='-', color='y')
    plt.xlabel('$date$', color=fg_color)
    plt.ylabel('$gain$', color=fg_color)
    plt.show()


def plot_curves(curves, days):
    '''
    Grafica dos curvas.
    '''
    bg_color = 'black'
